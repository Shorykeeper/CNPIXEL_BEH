# -*- coding: utf-8 -*-
# Created on 2020-04-20
from ..base_event import BaseEvent


class ActorHurtServerEvent(BaseEvent):
    """触发时机：生物（包括玩家）受伤时

    - entityId: str 生物Id
    - cause: str 伤害来源，详见Minecraft枚举值文档的ActorDamageCause
    - damage: int 伤害值
    - absorbedDamage: int 吸收的伤害值（原始伤害减少damage）
    """
    pass


class ActuallyHurtServerEvent(BaseEvent):
    """触发时机：实体实际受到伤害时，相比于DamageEvent，该伤害是经过吸收buff计算后的，实际的扣除量

    - srcId: str 伤害来源id
    - projectileId: str 投掷物id
    - entityId: str 受伤id
    - damage: int 伤害值，允许修改，设置为0则此次造成的伤害为0
    - damage_f: float 伤害值（被伤害吸收后的值），允许修改，如果修改该值，则会覆盖damage的修改效果
    - cause: str 伤害来源，详见Minecraft枚举值文档的ActorDamageCause
    """
    pass


class AddEffectServerEvent(BaseEvent):
    """触发时机：实体获得状态效果时

    - entityId: str 实体id
    - effectName: str 实体获得状态效果的名称
    - effectDuration: int 状态效果的持续时间，单位秒
    - effectAmplifier: int 状态效果的放大倍数
    - damage: int 状态效果造成的伤害值，如中毒
    """
    pass


class ChangeSwimStateServerEvent(BaseEvent):
    """触发时机：实体开始或结束游泳时

    - entityId: str 实体的唯一ID
    - formState: bool 事件触发前，实体是否在游泳状态
    - toState: bool 事件触发后，实体是否在游泳状态

    当实体的状态没有变化时，不会触发此事件，即formState与toState必须是一个真一个假
    """
    pass


class DamageEvent(BaseEvent):
    """触发时机：实体受到伤害时

    - srcId: str 伤害来源id
    - projectileId: str 投掷物id
    - entityId: str 受伤id
    - damage: int 伤害值，允许修改，设置为0则此次造成的伤害为0
    - absorption: int 伤害吸收生命值，详见AttrType的ABSORPTION
    - cause: str 伤害来源，详见Minecraft枚举值文档的ActorDamageCause
    - knock: bool 是否击退被攻击者，允许修改，设置该值为False则不产生击退
    - ignite: bool 是否点燃被攻击者，允许修改，设置该值为True则产生点燃效果，反之自然

    damage值会被吸收和absorption等吸收，不一定是最终扣除量。通过设置这个伤害值可以取消伤害，但不会取消由击退或点燃效果带来的伤害

    当目标无法被击退时，knock值无效
    """
    pass


class EntityChangeDimensionServerEvent(BaseEvent):
    """触发时机：实体维度变化时服务器端抛出

    - entityId: str 实体id
    - fromDimensionId: int 维度变化前的维度
    - toDimensionId: int 维度变化后的维度
    - fromX: float 变化前的坐标x
    - fromY: float 变化前的坐标Y
    - fromZ: float 变化前的坐标Z
    - toX: float 变化后的坐标x
    - toY: float 变化后的坐标Y
    - toZ: float 变化后的坐标Z

    实体转移维度时，如果对应维度的对应位置的区块尚未加载，实体会存储在维度自身的冲突区内，直到对应区块被加载时才会创建对应的实体，此事件的抛出仅代表实体从原维度丢失，并不代表一定会在对应维度出现
    """
    pass


class EntityDefinitionsEventServerEvent(BaseEvent):
    """触发时机：生物定义json文件中设置的event事件同时触发。生物行为为变更事件

    - entityId: str 生物id
    - eventName: str 触发的事件名称
    """
    pass


class EntityDieLoottableServerEvent(BaseEvent):
    """触发时机：生物死亡掉落物品时

    - dieEntityId: str 死亡实体的entityId
    - attacker: str 伤害来源的entityId
    - itemList: list(dict) 掉落物品列表，每个元素是一个itemDict，格式可参考物品信息字典
    - dirty: bool 默认为False，如果需要修改掉落列表需将该值设置为True

    只有当dirty为True时，才会重新读取item列表并生成对应的掉落物，如果不需要修改掉落结果的请勿随意修改dirty值
    """
    pass


class EntityEffectDamageServerEvent(BaseEvent):
    """生物受到状态伤害/恢复事件。

    - entityId: str 实体id
    - damage: int 伤害值（负数表示生命恢复）
    - attributeBuffType: int 状态类型，参考AttributeBuffType
    - duration: float 状态持续时间，单位秒（s）
    - lifeTimer: float 状态生命时间，单位秒（s）
    - isInstantaneous: bool 是否为即时生效状态
    """
    pass


class EntityLoadScriptEvent(BaseEvent):
    """数据库加载实体自定义数据时触发

    - args: list 该事件的参数为长度为2的list，而非dict，其中list的第一个元素为实体id

    只有使用过extraData组件的实体才有此事件，触发时可以通过extraData组件获取该实体的自定义数据
    """
    pass


class EntityStartRidingEvent(BaseEvent):
    """当实体骑乘另一个实体时触发

    - id: str 骑乘者实体id
    - rideId: str 被骑乘者实体id
    """
    pass


class EntityStopRidingEvent(BaseEvent):
    """触发时机：当实体停止骑乘时

    - id: str 实体id
    - rideId: str 坐骑id
    - exitFromRider: bool 是否下坐骑
    - entityIsBeingDestroyed: bool 坐骑是否将要销毁
    - switchingRides: bool 是否换坐骑
    - cancel: bool 设置为True可以取消（需要与客户端事件一同取消）

    以下情况不允许取消
        - ride组件StopEntityRiding接口
        - 玩家传送时
        - 坐骑死亡时
        - 玩家掉线时
        - 玩家死亡时
        - 未服的马
        - 海豚的实体进入水中
        - 切换维度
    """
    pass


class EntityTickServerEvent(BaseEvent):
    """触发时机：实体tick时。该事件为20毫秒每秒。需要使用AddEntityTickEventWhiteList添加触发此事件的实体类型白名单。

    - entityId: str 实体id
    - identifier: str 实体identifier
    """
    pass


class HealthChangeBeforeServerEvent(BaseEvent):
    """触发时机：生物生命值发生变化之前触发

    - entityId: str    实体id
    - from: float      变化前的生命值
    - to: float        将要变化到的生命值，cancel设置为True时可以取消该变化，但是此参数不变
    - byScript: bool   是否通过SetAttrValue或SetAttrMaxValue调用大的变化
    - cancel: bool     是否取消该变化
    """
    pass


class HealthChangeServerEvent(BaseEvent):
    """触发时机：生物生命值发生变化时触发

    - entityId: str 实体id
    - from: float 变化前的生命值
    - to: float 变化后的生命值
    - byScript: bool 是否通过SetAttrValue或SetAttrMaxValue调用大的变化
    """
    pass


class MobDieEvent(BaseEvent):
    """触发时机：实体被玩家击杀时

    - id: str 实体id
    - attacker: str 伤害来源id

    注意：不能在此事件回调中对该玩家手持物品进行修改，如SpawnItemToPlayerCarried、ChangePlayerItemTipsAndExtraId等接口
    """
    pass


class MobGriefingBlockServerEvent(BaseEvent):
    """触发时机：环境生物变化方块时触发，触发的时机与mobgriefing游戏规则影响的行行为相同

    - cancel: bool 是否允许触发，默认为False，如果设置为True，则可以阻止触发后的物理解交互事件
    - blockX: int 方块x坐标
    - blockY: int 方块y坐标
    - blockZ: int 方块z坐标
    - entityId: str 触发的entity的唯一ID
    - blockName: str 方块的identifier，包含命名空间及名称
    - dimensionId: int 维度id

    触发的时机包括：生物踩踏腐植土、破坏单个方块、破坏门、点燃TNT方块、憎恶boss破坏方块、末影龙破坏方块、末影人抓取方块、海豚的实体进入水中、切换维度
    """
    pass


class OnFireHurtEvent(BaseEvent):
    """触发时机：生物受到火焰伤害时触发

    - victim: str 受伤实体id
    - src: str 火焰创建者id
    - fireTime: float 着火焰时间，单位秒
    - cancel: bool 是否取消此火焰伤害
    """
    pass


class OnGroundServerEvent(BaseEvent):
    """触发时机：实体着地事件。实体，掉落的物品，点燃的TNT着地时触发

    - id: str 实体id
    """
    pass


class OnKnockBackServerEvent(BaseEvent):
    """触发时机：实体受到击退时触发

    - id: str 实体id
    """
    pass


class ProjectileCritHitEvent(BaseEvent):
    """触发时机：当投掷物头部暴击时触发事件。注意：需要调用OpenPlayerCritBox开启玩家暴击后才能触发

    - id: str 弹射id
    - targetId: str 击中目标id
    """
    pass


class ProjectileDoHitEffectEvent(BaseEvent):
    """触发时机：当投掷物击中时触发事件

    - id: str 弹射id
    - hitTargetType: str 击中目标类型,"ENTITY"或是"BLOCK"
    - targetId: str 击中目标id
    - hitFace: int 击中在方块上的面id，参考Facing枚举
    - x: float 击中x坐标
    - y: float 击中y坐标
    - z: float 击中z坐标
    - blockPosX: int 击中是方块时，方块x坐标
    - blockPosY: int 击中是方块时，方块y坐标
    - blockPosZ: int 击中是方块时，方块z坐标
    - srcId: str 创建者id
    """
    pass


class RefreshEffectServerEvent(BaseEvent):
    """触发时机：实体身上的状态效果更新时触发，更新条件1、新增状态等级较高，更新状态等级及时间；2、新增状态等级不变，时间较长，更新状态持续时间

    - entityId: str 实体id
    - effectName: str 更新状态效果的名称
    - effectDuration: int 更新后的状态效果剩余持续时间，单位秒
    - effectAmplifier: int 更新后的状态效果放大倍数
    - damage: int 状态效果造成的伤害值，如中毒
    """
    pass


class RemoveEffectServerEvent(BaseEvent):
    """触发时机：实体身上的状态效果被移除时

    - entityId: str 实体id
    - effectName: str 被移除状态效果的名称
    - effectDuration: int 被移除状态效果的剩余持续时间，单位秒
    - effectAmplifier: int 被移除状态效果的放大倍数
    """
    pass


class SpawnProjectileServerEvent(BaseEvent):
    """触发时机：投掷物生成时触发

    - projectileId: str 投掷物的实体id
    - projectileIdentifier: str 投掷物的identifier
    - spawnerId: str 发射者实体id，没有发射者时为-1
    """
    pass


class StartRidingServerEvent(BaseEvent):
    """触发时机：当实体即将骑乘另一个实体

    - cancel: bool 是否允许触发，默认为False，如果设置为True，则可以阻止触发后的实体交互事件
    - actorId: str 骑乘者实体的唯一ID
    - victimId: str 被骑乘者实体的唯一ID
    """
    pass


class WillAddEffectServerEvent(BaseEvent):
    """触发时机：实体即将获得状态效果

    - entityId: str 实体id
    - effectName: str 实体获得状态效果的名称
    - effectDuration: int 状态效果的持续时间，单位秒
    - effectAmplifier: int 状态效果的放大倍数
    - cancel: bool 设置为True可以取消
    - damage: int 状态效果造成的伤害值，如中毒；需要注意，该值不一定是最最终的伤害值
    """
    pass


class WillTeleportToServerEvent(BaseEvent):
    """触发时机：实体即将传送或切换维度

    - cancel: bool 是否允许触发，默认为False，如果设置为True，则可以阻止触发后的传送
    - entityId: str 实体的唯一ID
    - fromDimensionId: int 传送前所在的维度
    - toDimensionId: int 传送后的目标维度
    - fromX: int 传送前所在的坐标x
    - fromY: int 传送前所在的坐标Y
    - fromZ: int 传送前所在的坐标Z
    - toX: int 传送目标点的坐标x
    - toY: int 传送目标点的坐标Y
    - toZ: int 传送目标点的坐标z
    - cause: str 传送原因，详见MinecraftEnum.EntityTeleportCause

    假如目标维度尚未在内存中创建（即服务器启动之后，到传送之前，没有玩家进入过该维度），那么此事件中返回的目标点坐标是算法生成的，不能保证准确。
    """
    pass

